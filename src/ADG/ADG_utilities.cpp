...
namespace ses {
    ...
    int get_totalStateCnt(ADG &adg) { return get<2>(adg).back(); }
    ...
    bool is_type2_edge(ADG &adg, int agent1, int state1, int agent2, int state2) {
        ...
        return (get_type2_switchable_edge(graph, v1, v2) || get_type2_nonSwitchable_edge(graph, v1, v2));
    }
    ...
    Location get_state_target(ADG &adg, int agent, int state) { return get<0>(((get<1>(adg))[agent])[state]); }
