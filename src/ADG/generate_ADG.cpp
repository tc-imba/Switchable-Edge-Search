    ...
    void add_type1_edges(Graph &graph, Paths &paths, vector<int> &accum_stateCnts) {
        ...
        for (int agent = 0; agent < agentCnt; agent++) {
            ...
            for (int state = 0; state < stateCnt; state++) {
                ...
                if (prev_vertex >= 0) { set_type1_edge(graph, prev_vertex, curr_vertex); }
    ...
    ADG construct_delayed_ADG(ADG &adg, int dlow, int dhigh, vector<int> &delayed_agents, vector<int> &states,
        ...
        for (int agent = 0; agent < agentCnt; agent++) {
            if (delayed_agents[agent] == 0) {
                ...
                for (int state = 0; state <= delayed_state; state++) { new_path.push_back(ori_path[state]); }
                pair<Location, int> repeat = make_pair(get<0>(new_path.back()), -1);
                int delay = distrib(gen);
                outFile_setup << delay << "\n";
                delay_sum += delay;
                for (int state = 0; state < delay; state++) { new_path.push_back(repeat); }
                int ori_size = ori_path.size();
                for (int state = delayed_state + 1; state < ori_size; state++) { new_path.push_back(ori_path[state]); }
