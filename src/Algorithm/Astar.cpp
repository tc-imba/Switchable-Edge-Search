...
namespace ses {
    ...
    Astar::Astar(int input_timeout) { timeout = input_timeout; }
    ...
    void Astar::print_stats(ofstream &outFile) {
        outFile << explored_node_cnt << "," << pruned_node_cnt << "," << added_node_cnt << "," << vertex_cnt << ","
                << sw_edge_cnt << "," << heuristicT.count() << "," << branchT.count() << "," << sortT.count() << ","
                << pqT.count() << "," << copy_free_graphsT.count() << "," << termT.count() << "," << dfsT.count()
                << endl;
    }
    ...
    ADG Astar::exploreNode() {
        ...
        while (pq.size() > 0) {
            ...
            if (explored_node_cnt % 1000000 == 0) { std::cout << explored_node_cnt << "\n"; }
    ...
    ADG Astar::slow_exploreNode() {
        ...
        while (slow_pq.size() > 0) {
            explored_node_cnt += 1;
            if (explored_node_cnt % 100000 == 0) { std::cout << explored_node_cnt << "\n"; }
