...
namespace ses {
    ...
    void rem_type1_edge(Graph &graph, int n1, int n2) {
        ...
        if (itr != type1G.first[n1].end()) { type1G.first[n1].erase(itr); }

        itr = type1G.second[n2].find(n1);
        if (itr != type1G.second[n2].end()) { type1G.second[n2].erase(itr); }
    ...
    void rem_type2_nonSwitchable_edge(Graph &graph, int n1, int n2) {
        ...
        if (itr != type2NSG.first[n1].end()) { type2NSG.first[n1].erase(itr); }

        itr = type2NSG.second[n2].find(n1);
        if (itr != type2NSG.second[n2].end()) { type2NSG.second[n2].erase(itr); }
    ...
    void rem_type2_switchable_edge(Graph &graph, int n1, int n2) {
        ...
        if (itr != type2SG.first[n1].end()) { type2SG.first[n1].erase(itr); }

        itr = type2SG.second[n2].find(n1);
        if (itr != type2SG.second[n2].end()) { type2SG.second[n2].erase(itr); }

        return;
    }

    bool get_type1_edge(Graph &graph, int n1, int n2) {
        if (error_check_node_range(graph, n1, n2) == false) { return false; }
    ...
    bool get_edge(Graph &graph, int n1, int n2) {
        ...
        if (result == false) {
            ...
            if (result == false) { result = get_type2_switchable_edge(graph, n1, n2); }
        }
    ...
    set<int> &get_type2_nonSwitchable_inNeib(Graph &graph, int n) { return get<1>(graph).second[n]; }
    ...
    for(int i = 0; i < n; i++){
        ...
        set<int> *type2NonSwitchableGInNeighbors = new set<int>[n];
        for (int i = 0; i < n; i++) {
            copy(get<1>(graph).second[i].begin(), get<1>(graph).second[i].end(),
                 inserter(type2NonSwitchableGInNeighbors[i], type2NonSwitchableGInNeighbors[i].begin()));
        }
        set<int> *type2NonSwitchableGOutNeighbors = new set<int>[n];
        for (int i = 0; i < n; i++) {
            copy(get<1>(graph).first[i].begin(), get<1>(graph).first[i].end(),
                 inserter(type2NonSwitchableGOutNeighbors[i], type2NonSwitchableGOutNeighbors[i].begin()));
        }
        subGraph type2NonSwitchableG = make_pair(type2NonSwitchableGOutNeighbors, type2NonSwitchableGInNeighbors);

        set<int> *type2SwitchableGInNeighbors = new set<int>[n];
        for (int i = 0; i < n; i++) {
            copy(get<2>(graph).second[i].begin(), get<2>(graph).second[i].end(),
                 inserter(type2SwitchableGInNeighbors[i], type2SwitchableGInNeighbors[i].begin()));
        }
        set<int> *type2SwitchableGOutNeighbors = new set<int>[n];
        for (int i = 0; i < n; i++) {
            copy(get<2>(graph).first[i].begin(), get<2>(graph).first[i].end(),
                 inserter(type2SwitchableGOutNeighbors[i], type2SwitchableGOutNeighbors[i].begin()));
        }
        subGraph type2SwitchableG = make_pair(type2SwitchableGOutNeighbors, type2SwitchableGInNeighbors);
    ...
    void print_graph(Graph &graph) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            cout << i << ": ";
            for (auto itr = g.begin(); itr != g.end(); itr++) { cout << *itr << " "; }
            cout << endl;
        }
    ...
    bool check_cycle_NS_helper(Graph &graph, int current, vector<bool> &visited, vector<bool> &parents, bool type2_flag,
        ...
        if (type2_flag) {
            ...
            if (*itr != flag_parent && parents[*itr] == true) { return true; }
            bool result =
                    *itr != flag_parent ? check_cycle_NS_helper(graph, *itr, visited, parents, false, current) : false;
    ...
    bool check_cycle_NS_helper_old(Graph &graph, int current, vector<bool> &visited, vector<bool> &parents) {
        if (visited[current] == false) {
            ...
            for (auto itr = neighborhood.begin(); itr != neighborhood.end(); itr++) {
                if (parents[*itr] == true) { return true; }
                bool result = check_cycle_NS_helper_old(graph, *itr, visited, parents);
                if (result == true && !visited[*itr]) { return true; }
            }
    ...
    bool isCyclicUtil(Graph &graph, int v, vector<bool> &visited, vector<bool> &recStack) {
        if (visited[v] == false) {
            visited[v] = true;
            recStack[v] = true;
            ...
            for (auto i = neighbors.begin(); i != neighbors.end(); i++) {
                if (!visited[*i] && isCyclicUtil(graph, *i, visited, recStack)) return true;
                else if (recStack[*i])
                    return true;
            }
        }
    ...
    void build_time_arr(Graph &graph, vector<bool> &visited, vector<int> *sorted_vertecies, vector<int> *sorted_times,
